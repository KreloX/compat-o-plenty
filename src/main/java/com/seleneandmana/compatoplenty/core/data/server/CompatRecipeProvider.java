...
public class CompatRecipeProvider extends RecipeProvider {
    ...
    public CompatRecipeProvider(PackOutput output) {
        super(output);
    }
    ...
    protected void buildRecipes(@NotNull Consumer<FinishedRecipe> consumer) {
        for (var woodMaterial : WoodMaterial.WOOD_MATERIALS) {
            ...
            var slab = getItem.apply(woodName + "_slab");
            var planks = getItem.apply(woodName + "_planks");
            var leaves = getItem.apply(woodName + "_leaves");
        ...
        */
        ...
        flaggedFurnaceRecipe(BOPBlocks.GLOWING_MOSS_BLOCK, CompatItems.GLOWING_MOSS_PASTE.get(), 1.0f, MIDORI_FLAG, consumer);
    ...
    public static final ResourceLocation quarkFlag = new ResourceLocation("quark", "flag");

    private static Zeta2ForgeCondition<FlagCondition> quarkCondition(String flag) {
        return new Zeta2ForgeCondition<>(new FlagCondition(Quark.ZETA.configManager.getConfigFlagManager(), flag, quarkFlag, () -> false));
    }
    ...
    public static void sandstoneSlabRecipe(ItemLike material, ItemLike slab, Consumer<FinishedRecipe> consumer) {
        ConditionalRecipe.builder()
                .addCondition(quarkCondition(SANDSTONE_BRICKS_FLAG))
                .addRecipe(consumer1 -> slabBuilder(RecipeCategory.DECORATIONS, slab, ingredientOf(material)).unlockedBy(getHasName(material), has(material)).save(consumer1))
    ...
    public static void sandstoneWallRecipe(ItemLike material, ItemLike wall, Consumer<FinishedRecipe> consumer) {
        ConditionalRecipe.builder()
                .addCondition(quarkCondition(SANDSTONE_BRICKS_FLAG))
                .addRecipe(consumer1 -> wallBuilder(RecipeCategory.DECORATIONS, wall, ingredientOf(material)).unlockedBy(getHasName(material), has(material)).save(consumer1))
    ...
    public static void galanosRecipe(Consumer<FinishedRecipe> consumer) {
        ConditionalRecipe.builder()
                .addCondition(quarkCondition(MIDORI_FLAG))
                .addRecipe(consumer1 -> ShapedRecipeBuilder.shaped(RecipeCategory.DECORATIONS, GALANOS_BLOCK.get()).define('#', CompatItems.GLOWING_MOSS_PASTE.get()).pattern("##").pattern("##").unlockedBy(getHasName(CompatItems.GLOWING_MOSS_PASTE.get()), has(CompatItems.GLOWING_MOSS_PASTE.get())).save(consumer1))
    ...
    public static void quarkFlagStoneCutterRecipe(ItemLike material, ItemLike result, int amount, String flag, Consumer<FinishedRecipe> consumer) {
        ConditionalRecipe.builder()
                ...
                .addRecipe(consumer1 -> SingleItemRecipeBuilder.stonecutting(ingredientOf(material), RecipeCategory.DECORATIONS, result, amount).unlockedBy(getHasName(material), has(material)).save(consumer1))
    ...
    public static void flaggedFurnaceRecipe(ItemLike material, ItemLike result, float exp, String flag, Consumer<FinishedRecipe> consumer) {
        ConditionalRecipe.builder()
                .addCondition(quarkCondition(flag))
                .addRecipe(consumer1 -> SimpleCookingRecipeBuilder.smelting(ingredientOf(material), RecipeCategory.DECORATIONS, result, exp, 200).unlockedBy(getHasName(material), has(material)).save(consumer1))
