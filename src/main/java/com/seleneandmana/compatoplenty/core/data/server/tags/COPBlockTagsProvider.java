    ...
    public void addTags(Provider provider) {
        for (var woodMaterial : WoodMaterial.WOOD_MATERIALS) {
            tag(BlockTags.MINEABLE_WITH_AXE).add(
                    COPBlocks.VERTICAL_SLABS.get(woodMaterial).get(),
                    COPBlocks.POSTS.get(woodMaterial).get(),
                    COPBlocks.STRIPPED_POSTS.get(woodMaterial).get(),
                    COPBlocks.HEDGES.get(woodMaterial).get(),
                    COPBlocks.CABINETS.get(woodMaterial).get(),
                    COPBlocks.VERTICAL_PLANKS.get(woodMaterial).get()
            );
            tag(BlockTags.PLANKS).add(COPBlocks.VERTICAL_PLANKS.get(woodMaterial).get());
            tag(BlueprintBlockTags.WOODEN_CHESTS).add(COPBlocks.CHESTS.get(woodMaterial).get());
            tag(BlueprintBlockTags.WOODEN_TRAPPED_CHESTS).add(COPBlocks.TRAPPED_CHESTS.get(woodMaterial).get());
            tag(BlueprintBlockTags.WOODEN_LADDERS).add(COPBlocks.LADDERS.get(woodMaterial).get());
            tag(BlueprintBlockTags.WOODEN_BEEHIVES).add(COPBlocks.BEEHIVES.get(woodMaterial).get());
            tag(BlueprintBlockTags.WOODEN_BOOKSHELVES).add(COPBlocks.BOOKSHELVES.get(woodMaterial).get());
            tag(BlueprintBlockTags.WOODEN_CHISELED_BOOKSHELVES); // TODO
            tag(BlueprintBlockTags.WOODEN_BOARDS).add(COPBlocks.BOARDS.get(woodMaterial).get());
            tag(BlueprintBlockTags.LEAF_PILES).add(COPBlocks.LEAF_PILES.get(woodMaterial).get());
            tag(TwigsTags.TABLES_BLOCK).add(COPBlocks.TABLES.get(woodMaterial).get());
            tag(TagUtil.blockTag("quark", "hollow_logs")); // TODO
            tag(TagUtil.blockTag("quark", "vertical_slabs")).add(COPBlocks.VERTICAL_SLABS.get(woodMaterial).get());
            tag(TagUtil.blockTag("quark", "hedges")).add(COPBlocks.HEDGES.get(woodMaterial).get());
        }
        // Minecraft tags
        tag(BlockTags.MINEABLE_WITH_AXE).add(
